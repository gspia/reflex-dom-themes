...
{-|
Module      : Reflex.Dom.Component.BS.Dropdown
...
module Reflex.Dom.Component.BS.Dropdown where
...
mkDropdown trMic dInitElms dLang updLang = do
    ...
    let fs1Lvl = CommonADEfuns listenMe actMU drawDDMI elemEvF
        ...
        rootConf = defRootDivButtonFuns
            ...
            & set rootfNodeAttr buttonRootAttrs
  ...
  where
    ...
    divRootAttrs _dAst _an = pure $ A.attrMap $  A.setClasses
        [bsDropdown] E.defDiv
    buttonRootAttrs ∷ ActiveState t ActNode (MenuItem t a)
         → Dynamic t A.Attr
    buttonRootAttrs ast = fmap (\idTxt →
                                A.attrMap
                                  ( A.setClasses [bsBtn, bsBtnSecondary, bsDropdownToggle]
                                  $ A.id_ idTxt
                                  $ A.dToggle "dropdown"
                                  $ A.btButton
    ...
    divAttr trA dAst an = fmap (\cls → A.attrMap $ A.setClasses cls
      ...
      where
        dCls = fmap (\b →
                    if b then [bsDropdownMenu, bsShow] else [bsDropdownMenu]
                    ) dIsAct 
        dIsAct = join $ (\an2 → if an == _activeStateElemId an2
...
customDDMkLevelN lvlFs dmlst evB trSt pth treeE = do
    ...
    -- dynText dLbl
    let divCl = A.setClasses [bsCollapse, bsNavbarCollapse]
