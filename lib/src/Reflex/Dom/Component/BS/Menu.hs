...
{-|
Module      : Reflex.Dom.Component.BS.Menu
...
module Reflex.Dom.Component.BS.Menu where
...
mkMenuBarCommon mkL1 trMic dLang updLang = do
  ...
  where
        ...
        if isLeafAtPath trA pth
            then defLiLeafNodeAttrF
              (ast & set activeStateActiveGl
                ( A.setClasses [bsNavItem,bsActive] $ pure A.defGlobals)
               & set activeStateNotActiveGl
                ( A.setClasses [bsNavItem] $ pure A.defGlobals)
              )
            else defLiLeafNodeAttrF
              (ast & set activeStateActiveGl
                ( A.setClasses [bsNavItem,bsActive,bsDropdown] $ pure A.defGlobals)
               & set activeStateNotActiveGl
                ( A.setClasses [bsNavItem,bsDropdown] $ pure A.defGlobals)
              )
      where
    ...
    ulAttr ∷ Dynamic t (ActiveState t ActNode (MenuItem t a))
           → ActNode → Dynamic t A.Attr
    ulAttr _dAst _an =
        pure $ A.attrMap $ A.setClasses [bsNavbarNav, bsMrAuto]  E.defUl
    ...
    f ← mapM (\mtc@(Node rmic streec) →
      if null streec
         then do
           mi ← mkMenuItem rmic
           pure $ Node mi []
         -- else E.li aLi $ drawDropdownTreeNavbar "navbarMIforest" mtc
         -- TODO yllä oleva tuo mukanaan monta metodia...
         else E.li aLi $ text "tähän drawDropdownTreeNavbar"
      ) mfc
    pure $ FForest f
  where
    aLi = A.setClasses [bsNavItem, bsDropdown] E.defLi
    aUl = A.setClasses [bsNavbarNav, bsMrAuto] E.defUl
    -- hR u = href (URL u)
...
mkMenuItem mic = do
  ...
  let dLi = setActLi <$> dActive <*> constDyn E.defLi
      setActLi b ea =
        if b
           then A.setClasses [bsNavItem, bsActive] ea
           else A.setClasses [bsNavItem] ea
  E.liD dLi $ do
    (e,_) ← E.a' (A.setClasses [bsNavLink] E.defA) $ do
      if null ico
         then blank
         else E.span (A.setClasses ico E.defSpan) blank
      dynText dLabel
