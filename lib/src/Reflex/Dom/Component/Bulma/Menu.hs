...
{-|
Module      : Reflex.Dom.Component.Bulma.Menu
...
module Reflex.Dom.Component.Bulma.Menu where
...
mkMenuBarCommon mkL1 trMic dLang updLang = do
    ...
    let fs1Lvl = CommonADEfuns listenMe actMU drawMI elemEvF
        ...
        lvl1Fs = defLevelNNoneAFuns -- None A Div
        ...
        lvlNFs = defLevelNDivAFuns -- Dropdowns on the main menu Div A Div
...
customMkLevelLst ∷ forall t m a . (Reflex t, DomBuilder t m, PostBuild t m
...
customMkLevelLst lvlFs dmlst evB trSt pth treeE = do
    ...
    (ceA,ceB) ← E.div (A.setClasses [B.navbarBrand] E.defDiv) $ do
        (eA1,_) ← E.a' aAttrs $ do
            ...
            --            else E.span (A.setClasses [B.icon] E.defSpan) $
            --                     E.i (A.setClasses ico E.defI) blank
            dynText dLbl -- how to add I18n-support?
            -- pure dMic
    ...
    rec
        ...
        let eCas = () <$ leftmost (fmap _ceMUp cas)
